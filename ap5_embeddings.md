...
**Encoding goal**: Transform variable-length text $[w_ 1, w_ 2, ..., w_ N]$ into fixed-size vector $\vec{E}_ {\text{query}} \in \mathbb{R}^d$ (typically $d=128$ or $d=256$).
...
- **Output**: Sequence of contextualized vectors $[\vec{h}_ 1, \vec{h}_ 2, ..., \vec{h}_ N]$ where each $\vec{h}_ i$ represents token $i$ in the context of the full query
...
**Common strategies:**
- **Average pooling**: $\vec{E}_ {\text{query}} = \frac{1}{N} \sum_ {i=1}^{N} \vec{h}_ i$ (most common, robust)
- **Max pooling**: $\vec{E}_ {\text{query}} = \max(\vec{h}_ 1, \vec{h}_ 2, ..., \vec{h}_ N)$ (element-wise max)
...
2. **Contextualization**: Generate contextualized representations
   - $\vec{h}_ 3^{\text{hotel}}$ now encodes "hotel" in the context of "family", "best", and "Orlando"—capturing the concept of "high-quality family accommodation in Orlando"
