...
where $y\_i \in \{0, 1\}$ is the click label and $\hat{p}\_i$ is the predicted CTR.
...
- **Importance weighting**: During loss calculation, up-weight negative examples by the downsampling factor to correct for sampling bias:
  ...
  where $w\_i = 1$ for clicks, $w\_i = \text{downsample\\_ratio}$ for non-clicks.
...
- **At request time**:
  ...
  2. Run fusion head + prediction heads: $(E\_{\text{user}}, E\_{\text{context}}, E\_{\text{ad}}) \rightarrow (\text{pCTR}, \text{pCVR})$
